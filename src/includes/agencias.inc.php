<!-- Start Agências -->
<section id="agencias" class="section-nav">
	<h1 class="section-title">Agências</h1>
	<div class="agency">
		<div class="agency-row">
			<div class="agency-col-intro">
				<h4 class="agency-intro">Conheça as agências de checagem que são parceiras do <strong>Programa de Enfrentamento a Desinformação nas Eleições 2020</strong></h4> </div>
			<div class="agency-col-content">
				<div class="agency-body">
					<p class="agency-text"> Desde setembro de 2020, a página Fato ou Boato é a plataforma do projeto Coalizão para Checagem - Eleições 2020, que é parte do Programa de Enfrentamento à Desinformação e estabelece uma rede nacional de verificação de informações relativas ao processo eleitoral. Tendo como maior objetivo o enfrentamento das fake news, nove das principais agências de checagem do Brasil integram essa força-tarefa em favor da circulação de conteúdos verificados, que efetivamente promovam debates e esclarecimentos fundamentais à tomada de decisão do eleitor. </p>
				</div>
			</div>
			<div class="agency-deck">
				<div class="agency-deck-item">
					<a target="_blank" href="https://piaui.folha.uol.com.br/lupa"> <img src="imagens/logos/lupa.png" alt="Agência Lupa" title="Agência Lupa" /> </a>
				</div>
				<div class="agency-deck-item">
					<a target="_blank" href="https://politica.estadao.com.br/blogs/estadao-verifica"><img src="imagens/logos/estadao.png" alt="Estadão Verifica" title="Estadão Verifica" /> </a>
				</div>
				<div class="agency-deck-item">
					<a target="_blank" href="https://www.boatos.org"><img src="imagens/logos/boatos.png" alt="Boatos.org" title="Boatos.org" /> </a>
				</div>
				<div class="agency-deck-item">
					<a target="_blank" href="https://www.afp.com/pt"> <img src="imagens/logos/afp.png" alt="AFP" title="AFP" /> </a>
				</div>
				<div class="agency-deck-item">
					<a target="_blank" href="https://www.e-farsas.com"> <img src="imagens/logos/e-farsas.png" alt="E-Farsas" title="E-Farsas" /> </a>
				</div>
				<div class="agency-deck-item">
					<a target="_blank" href="https://projetocomprova.com.br"> <img src="imagens/logos/comprova.png" alt="Comprova" title="Comprova" /> </a>
				</div>
				<div class="agency-deck-item">
					<a target="_blank" href="https://noticias.uol.com.br/confere"><img src="imagens/logos/uol.png" alt="UOL Confere" title="UOL Confere" /></a>
				</div>
				<div class="agency-deck-item">
					<a target="_blank" href="https://www.aosfatos.org"> <img src="imagens/logos/aos-fatos.png" alt="Aos Fatos" title="Aos Fatos" /> </a>
				</div>
				<div class="agency-deck-item">
					<a target="_blank" href="https://g1.globo.com/fato-ou-fake"> <img src="imagens/logos/fato-fake.png" alt="Fato ou Fake" title="Fato ou Fake" /> </a>
				</div>
			</div>
		</div>
	</div>
</section>
<!-- End Agências -->
